...
// group functions
function is_groupname_available($groupName) {
    if (num_rows_matching("SELECT groupName FROM Groups WHERE groupName = '$groupName';") > 0) {
        return FALSE;
    }
    return TRUE;
}
...
// return only currently closed polls for the user
function get_closed_polls_for_user($userID) {
    return get_list_from_column("SELECT P.pollID
                                    FROM Polls P
                                    WHERE P.isOpen = 0 AND P.groupID IN (SELECT G.groupID
                                                                            FROM GroupMembership G
                                                                            WHERE G.userID = $userID)", "pollID");
}
